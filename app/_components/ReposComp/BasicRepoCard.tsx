...
export default function BasicRepoCard({ title, desc, repoUrl, style, status, }: basicRepoCardType) {
  return (
    <Link
      ...
      title={title}
      className={`w-full `}>
      <article className={`w-full grid gap-2 p-3 py-5 sm:p-5 border-b border-gray-500/20 hover:bg-very-light dark:hover:bg-very-dark dark:text-white text-black mx-auto ${style}`}>
        <h2
          title={title}
          className={` font-semibold text-2xl md:text-3xl capitalize`}>
          {title}
        </h2>
