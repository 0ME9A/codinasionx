"use client";
import { filterUniqueCondinasionDevs, filterUniqueRepos } from "app/_functions/filters";
...
export default function HomeAside() {
    ...
    const { data: repos, status: repoStatus } = useSelector((state: RootState) => state.counter.repositories)
    ...
    return (
        <Aside>
            <Box title={"Codinasion repositories"} >
                <p className="text-gray-700 dark:text-gray-400 ">{commonData.repoDesc}</p>
            </Box>
            {devStatus === 200 && <Contributors contributors={filteredContributors} />}
