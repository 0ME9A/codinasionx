"use client";
import { goodFirstIssueType } from "app/_types/GoodFirstIssue";
...
export default function HomeArticle() {
  ...
  const issueProp = useSelector(
    (state: RootState) => state.counter.issueProperties
  );
  const filtered =
    status === 200 ? filterIssue(data, issueProp.tag, search.keywords) : [];
  const [isMount, setMount] = useState(false);

  useEffect(() => {
    dispatch(issueProperties({ type: LIMIT, value: 1 }));
  }, [issueProp.tag, search.keywords]);
  useInfinite({
    funcVal: issueProperties({ type: LIMIT, value: issueProp.limit + 1 }),
    funcCall: dispatch,
  });
  ...
  return (
    <ArticleSection>
      ...
      {filtered &&
        filtered
          .slice(0, issueProp.limit * 10)
          .map((item: goodFirstIssueType) => (
