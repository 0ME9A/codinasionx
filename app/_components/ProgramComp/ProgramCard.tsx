  ...
  return (
    <Link href={parentPath + programData.slug} className={`flex`}>
      <article title={title} className={`w-full space-y-4 text-black dark:text-white p-3 sm:p-5 !py-10 hover:bg-very-light dark:hover:bg-very-dark border-b-[1px] border-gray-500/20 mx-auto ${style}`}>
        <h2 title={title} className="text-2xl font-semibold tracking-tight capitalize">{title}</h2>
        ...
        <ul className="md:flex md:space-x-10 text-md">
          <li title={`Total contributers ${programData.contributors.length}`} className={`flex items-center gap-2`}>
          ...
          <li title={`Last updated ${dateFunc(programData.last_updated)}`} className={`flex items-center gap-2`}>
