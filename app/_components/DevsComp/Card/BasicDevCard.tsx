...
export default function BasicDevCard({ data: { name, avatar, role = "contributor" } }: { data: codinasionDevType }) {
  return (
    <Link
      ...
      title={name}
      className="space-y-1 h-fit p-1 hover:shadow-lg border-2 border-transparent hover:border-darkII relative group dark:shadow-dark rounded-2xl overflow-hidden hover:bg-light dark:hover:bg-dark">
      <Image
        src={avatar}
