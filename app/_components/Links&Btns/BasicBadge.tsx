...
export default function Badge({ BadgeData: { text, highlightText, style, funcVal, clickFunc, funcState } }: badgeType) {
    return (
        <button
            type={"button"}
            title={`${text} ${highlightText}`}
            onClick={() => {clickFunc(funcVal)}}
            className={` rounded-xl relative block py-1 overflow-hidden h-full text-black dark:text-white hover:bg-very-light dark:hover:bg-very-dark group border border-yellow-500`}
